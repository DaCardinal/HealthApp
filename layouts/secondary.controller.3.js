(function(){
  ...
  function addTab(appBase, id, dir, title, tabHandler, wrapper, preloadJS){
    ...
    sTabHandler.pageTabGroup[id].on("tab-added", function(tab){
      console.log("tab-added", tab);
    });

    sTabHandler.pageTabGroup[id].on("tab-active", function(tab){
          ...
          sidebarTabHandler.pageTabGroup[el] = sidebarGroup.addElement({
              ...
              ready: function (tab) {
                ...
                tab.on("click", function(tab){
                  let activeTab = tab.tabGroup.getActiveTab();
